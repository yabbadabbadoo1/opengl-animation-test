...
int i = 0;
void tris( int a, int b, int c, const point3 &normal)
{
	pyramidPoints[i] = pvertices[a]; pyramidNormals[i] = normal;
	pyramidUV[i] = point2(0.0, 1.0f); i++;
	pyramidPoints[i] = pvertices[b]; pyramidNormals[i] = normal;
	pyramidUV[i] = point2(0.0, 0.0f); i++;
	pyramidPoints[i] = pvertices[c]; pyramidNormals[i] = normal;
	pyramidUV[i] = point2(1, 0.0f); i++;
}

void colorPyramid()
{
	tris( 0, 1, 4, point3( -1, 1, 0));
	tris( 1, 2, 4, point3( 0, 1, 1  ));
	tris( 2, 3, 4, point3( 1, 1, 0 ));
	tris( 3, 0, 4, point3( 0, 1, -1 ));
}
...
void colorbot()
{
	...
	pyramidPoints[i] = pvertices[0]; pyramidNormals[i] = sqnormal();
	pyramidUV[i] = point2(0.5f, 1.0f); i++;
	pyramidPoints[i] = pvertices[1]; pyramidNormals[i] = sqnormal();
	pyramidUV[i] = point2(0.5f, 0.0f); i++;
    pyramidPoints[i] = pvertices[2]; pyramidNormals[i] = sqnormal();
    pyramidUV[i] = point2(1.0f, 0.0f); i++;
    pyramidPoints[i] = pvertices[0]; pyramidNormals[i] = sqnormal();
    pyramidUV[i] = point2(0.5f, 1.0f); i++;
    pyramidPoints[i] = pvertices[2]; pyramidNormals[i] = sqnormal();
    pyramidUV[i] = point2(1.0f, 0.0f); i++;
    pyramidPoints[i] = pvertices[3]; pyramidNormals[i] = sqnormal();
    pyramidUV[i] = point2(1.0, 1.0f); i++;
}
